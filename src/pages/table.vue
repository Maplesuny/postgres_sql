...
let url = 'http://127.0.0.1:8000/api/v1/test?table=AllergyRecordDim'
...
get_data()
// console.log('ddd', ddd)
// console.log('ddddr', originalRows)
...
export default {
    setup () {
        ...
        function fetchFromServer (startRow, count, filter, sortBy, descending) {
            const data = filter
                ? ddd.filter(row => row.name.includes(filter))
                : ddd.slice()
            console.log('fetchFromServer_ data', data)
            // handle sortBy
            if (sortBy) {
            ...
            // 最後產出的是查出來的object 用list包起來
            console.log('retunr data', data.slice(startRow, startRow + count))
        ...
        function getRowsNumberCount (filter) {
            ...
            ddd.forEach(treat => {
                ...
                if (treat.name.includes(filter)) {
                    ...
                    ++count
                    console.log(count)
                    console.log('(treat.name.includes(filter)', treat.name.includes(filter))
                }
            })
            return count
        }
        ...
        function onRequest (props) {
            ...
            setTimeout(() => {
                ...
                console.log('rows.value.splic', rows.value)
